...
# Load error patterns from the config file, ignoring comments and preserving multi-word patterns
ERROR_PATTERNS=$(grep -v '^\s*#' "$CONFIG_FILE" | awk '{$1=$1;print}' | sort -u)
...
for log in $SYSLOG_PATTERN; do
    if [[ -f "$log" && -r "$log" ]]; then
        ...
        for pattern in $ERROR_PATTERNS; do
            # Search for lines that match the pattern AND contain an IP address
            errcnt=$(grep -Ei "$pattern" "$log" | grep -Eo "([0-9]{1,3}\.){3}[0-9]{1,3}" | wc -l)
            
            # Only report the pattern if there are matches related to IP addresses
            if [[ $errcnt -gt 0 ]]; then
                echo "Total number of occurrences for '$pattern' (related to IPs) : $errcnt" >> "$report"
            else
                echo "No matches for pattern '$pattern' related to IPs in $log" >> "$report"
            fi
            echo "" >> "$report"  # Blank line for clarity
        done
